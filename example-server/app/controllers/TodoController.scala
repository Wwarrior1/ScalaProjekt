...
object TodoController extends Controller{
  ...
  implicit val jsonReader = (
    (__ \ 'txt).read[String](minLength[String](1)) and
  ...
  // Creates a new feature that will handle any matching throwable that this feature might contain.
  // If there is no match or if this feature contains a valid result then the new feature will contain the same.
